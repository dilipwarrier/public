...
Apache Spark is a unified computing engine and a set of libraries for
...
In Pandas, you can use the column name to select a column and use
Python slices to get a selection of rows.

#+BEGIN_SRC python
  df1_pd.DEST_COUNTRY_NAME[0:4]

  # Extract rows 0 through 4 for the named columns using loc
  df1_pd.loc[0:5,["ORIGIN_COUNTRY_NAME", "count"]]

  # Extract rows 0 through 4 for columns 1 through 2 using iloc
  df1_pd.iloc[0:5,1:3]

  # Get fast access to a scalar value
  df1_pd.at[0,"ORIGIN_COUNTRY_NAME"]

  df1_pd.iat[0, 0]
#+END_SRC

A Spark expression is an SQL-like query that takes as
...
In Spark, you can filter records using filter() or, equivalently, where().
...
In Pandas, agg() and transform() can be used with Python lambda
functions.

The following expression calculates the per-invoice sums and counts of
quantities sold.

#+BEGIN_SRC python
  # Calculate the sum of quantities in each invoice
  df2_pd_gp = df2_pd.loc[:, ["InvoiceNo", "Quantity"]].groupby("InvoiceNo")
  df2_pd_gp.agg(["count", "sum"])
#+END_SRC
...
#+BEGIN_SRC python
  ...
  d1_pd.set_index('Age').join(d2_pd.set_index('Age'), how = "inner")
...
Spark uses the Parquet file format by default. This is an open-source
colum-oriented file format that provides several optimizations
especially for analytics workloads.

Parquet files store the schema for the data along with the data.
...
while Spark operates in the TB-EB range. Further, Pandas typically
runs locally on your PC while Spark is optimized to run in a
distributed fashion on computing clusters.
...
*** DONE Remove C:/MSys2/tmp/Spark installation and delete [[id:8f83b767-d1ad-4494-a08c-262b2911b798][doc section]] if everything is working smoothly
SCHEDULED: <2025-11-23 Sun>
:PROPERTIES:
:EFFORT:  00:15
:BENEFIT: 10
:RATIO: 0.40
:ARCHIVE_TIME: 2025-11-24 Mon 13:56
:END:
