...
Reached [[https://www.coursera.org/learn/implementation-of-genai-agents/lecture/p1JZF/structuring-your-agents-knowledge-base][here]].
...
The input parameter temperature controls how adventurous the model is when selecting tokens:
lower values (around 0–0.3) make output more deterministic and consistent,
while higher values (around 0.7–1.3) increase creativity and variability.

The parameter top_p, or nucleus sampling, limits choices to the smallest set of likely
tokens whose cumulative probability is less than or equal to p. In practice,
adjust only one parameter at a time: use temperature for simpler control,
and top_p when you want probability-based filtering of token choices.
