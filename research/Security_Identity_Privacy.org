...
It uses asymmetric cryptography with two keys - a private key that is
known only to the signer and a public key that the signer shares with
everyone. The private key is used to create the signature while the
public key is used to verify it.

The process consists of the following steps.

- Hashing the message :: The message is run through a cryptographic
  hash function such as SHA-256, producing a fixed-length digest.
- Creating the signature :: The signer’s private key encrypts the hash
  to produce the signature.
- Verification :: The verifier decrypts the signature with the public key and
  compares the result to a freshly computed hash from the message. If they match,
...
Authentication and integrity are provided by digital
signatures. Confidentiality is provided by encryption.
...
- Example:
  When logging into a bank website, HTTPS (TLS) encrypts your
  credentials so attackers cannot read them in transit.

- Analogy:
  Like sending a sealed envelope through the mail; no one can read it
  while it is being delivered.

** Encryption at Rest

This protects data while it is stored on a disk, database, or cloud storage.
